...
function aiPostureTable(k) {
  // Counts are descriptive: a zero means no AI-scoped evidence was ingested,
  ...
  return wideTable(tableHtml([
    ...
    { label: "Evidence", align: "left", get: (r) => `<span class="muted">${esc(r.Note)}</span>` },
  ], rows));
}
...
function renderAi(p) {
  ...
  content.innerHTML = `
    ...
    <div class="panel-grid">
      ...
      ${panelHtml("ai-posture", 12, "Commitment &amp; rate posture", "Whether AI/ML spend is on a commitment, and what AI-scoped rate evidence exists.", aiPostureTable(k))}
...
function capacityNavigationHtml() {
  return `<nav class="capacity-tabs" role="tablist" aria-label="Capacity evidence classes">
...
function capacityHomeTable(classes) {
  ...
  return `<div class="capacity-table-scroll"><table class="dtable capacity-index">
    <thead><tr><th scope="col">Evidence class</th><th scope="col">Type</th><th scope="col">Observations</th><th scope="col">Resources</th><th scope="col">Snapshot days</th><th scope="col">Last seen</th></tr></thead>
    <tbody>${rows.map((item) => {
      const summary = item.summary || {};
      const observed = Number(summary.Observations || 0) > 0;
      return `<tr>
        <td><button type="button" class="capacity-link" data-capacity-class="${esc(item.id)}">${esc(item.title)}</button><div class="capacity-evidence">${esc(item.evidenceLabel)}</div></td>
        <td>${esc(item.evidenceClass === "inventory" ? "Inventory" : "Provider metric")}</td>
        <td>${observed ? fmtInt(summary.Observations) : "No evidence"}</td>
...
function capacityCurrentTable(payload) {
  const inventory = payload.contract?.evidenceClass === "inventory";
  ...
  const columns = [
    ...
    { label: "Evidence state", align: "left", get: (row) => `${capacityStateToken(row.semantic)}<span class="capacity-reason">${esc(row.semantic?.evidenceLabel || "")}</span>` },
...
export function capacityHeatmapCell(row, classId) {
  ...
  return {
    ...
    text: Number.isFinite(semantic.utilizationPercent) ? `${semantic.utilizationPercent.toFixed(1)}%` : (CAPACITY_STATE_LABELS[semantic.state] || "No evidence"),
...
function capacityHeatmap(payload) {
  ...
  if (!rows.length) return `<div class="capacity-notice">No heatmap evidence is available for this selection.</div>`;
  if (payload.contract?.evidenceClass !== "inventory" && !rows.some((row) => row.semantic?.capability === "enabled")) {
  ...
  return `<div class="capacity-table-scroll"><table class="capacity-heatmap">
    ...
    <tbody>${subscriptions.map((subscription) => `<tr>
      ...
      ${regions.map((region) => {
        ...
        if (!row) return `<td class="capacity-cell capacity-state--missing">No evidence</td>`;
...
function capacityHistory(payload) {
  ...
  if (history.mode === "current-only") {
    return `<div class="capacity-notice"><strong>Collecting ${payload.contract?.evidenceClass === "inventory" ? "inventory" : "quota"} history — 1 day available.</strong> Trend, growth, forecast, runway, and breach dates remain disabled.</div>`;
  }
  return tableHtml([
    { label: "UTC day", align: "left", get: (row) => esc(String(row.Day || "").slice(0, 10)) },
    { label: "Current", get: (row) => formatCapacityValue(row.currentValue) },
    { label: "Limit", get: (row) => payload.contract?.evidenceClass === "inventory" ? "Not applicable" : formatCapacityValue(row.limit) },
...
function capacityDemandHistory(payload) {
  ...
  ], series.points || [], "No billed-demand evidence matched this exact series.");
}
...
function capacityReconciliation(payload) {
  ...
  ], rows, "No capacity reservation inventory or linked billing evidence is available.");
}
...
function renderCapacity(payload) {
  ...
  if (payload.classId === "home") {
    content.innerHTML = `${nav}<section id="capacity-panel" role="tabpanel" aria-labelledby="capacity-tab-home">
      <div class="capacity-header">
        <div><span class="capacity-eyebrow">Capacity evidence</span><h2>Capacity workspace</h2></div>
        <p>Quota entitlement, billed demand, inventory, physical supply, and pricing commitments are separate evidence paths.</p>
      </div>
  ...
  const noEvidence = coverage.state === "no-evidence"
    ? `<div class="capacity-notice capacity-notice--warning"><strong>No evidence — collection outcome unknown.</strong> ${esc(payload.contract?.emptyLabel || "")}</div>`
    : "";
  ...
  const kpis = [
    ...
    kpiCard("Unclassified", fmtInt(statusCounts.unclassified), "Raw evidence retained; registry review required"),
  ...
  content.innerHTML = `${nav}<section id="capacity-panel" role="tabpanel" aria-labelledby="capacity-tab-${esc(payload.classId)}">
    <div class="capacity-header">
      <div><span class="capacity-eyebrow">${esc(payload.contract?.evidenceClass || "evidence")}</span><h2>${esc(payload.contract?.title || payload.classId)}</h2></div>
      <p>${esc(payload.capability?.evidenceLabel || payload.contract?.evidenceLabel || "")}</p>
    </div>
    ${noEvidence}${schemaNotice}
    <div class="capacity-notice"><strong>Next action:</strong> ${esc(CAPACITY_ACTIONS[payload.classId] || "Review the source evidence before taking action.")}</div>
    <div class="kpi-grid">${kpis}</div>
    ...
    <div class="capacity-layout">
      ${capacityPanel("Current evidence", `${payload.table?.rowLimit || 250}-row bound${payload.table?.truncated ? " reached" : ""}. Raw rows remain visible when calculations are disabled.`, capacityCurrentTable(payload))}
      ...
      ${capacityPanel("Parallel billed demand", payload.demand?.capability?.evidenceLabel || "Financial and usage evidence remains separate from quota.", capacityDemandHistory(payload))}
...
function updateChrome() {
  ...
  if (w && w.dataMin) {
    ...
    el("source-line").innerHTML =
      `Hub capacity evidence · <code>${esc(window.__cfg?.clusterUri || "localhost:8082")}</code>`;
    el("footer-meta").textContent = p.classId === "home" ? "seven evidence classes" : p.contract?.title || p.classId;
    queryState.rows = observations;
    queryState.health = p.error ? "error" : observations > 0 ? "ok" : "warn";
    queryState.refreshedAt = p.generatedAt ? new Date(p.generatedAt) : new Date();
    queryState.dataset = p.classId === "home" ? "Capacity evidence index" : p.contract?.title || "Capacity evidence";
    renderDiagnosticRail();
  }
}
