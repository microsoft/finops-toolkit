...
Demo projects keep the data source saved in the semantic model and read open data from `src/open-data` in the current working tree, so the demo is built from the branch being released. Templates always ship with the data source set to null and the release open data URL.
