...
# Path config — these MUST be populated by configure() before any rendering work.
# No defaults, no fallbacks. Touching SOURCE/LABELS/V8/OUT before configure() is
# called raises an AttributeError or TypeError immediately, surfacing the bug.
SOURCE = None       # path to source-template.pptx (required)
LABELS_PATH = None  # path to labels.yaml (required)
OUT = None          # default output .pptx path (optional, build_yaml.py sets per-invocation)
V8 = None           # path to deck-outline-v8.md (only needed when running build.py against the .md outline)
ASSETS = ROOT / "assets"   # for charts referenced by `screens:` directives — co-located with build.py
CHARTS = ROOT / "charts" / "svg"


def configure(template, labels, charts_dir=None, assets_dir=None, outline_md=None, output=None):
    """Populate path globals + LABELS dict. Required before any render call.

    Args:
        template:    path to source-template.pptx (required)
        labels:      path to labels.yaml (required)
        charts_dir:  path to chart SVG/PNG dir (required if any slide uses screens: chart=...)
        assets_dir:  path to image asset dir (required if any slide uses screens: image=...)
        outline_md:  path to deck-outline-v8.md (only when running build.py against the .md)
        output:      default output .pptx path (optional)
    """
    global SOURCE, LABELS_PATH, V8, OUT, LABELS, CHARTS, ASSETS
    if not template:
        sys.exit("ERROR: build.configure() requires `template` (path to source-template.pptx)")
    if not labels:
        sys.exit("ERROR: build.configure() requires `labels` (path to labels.yaml)")
    SOURCE = Path(template).expanduser().resolve()
    if not SOURCE.exists():
        sys.exit(f"ERROR: template not found: {SOURCE}")
    LABELS_PATH = Path(labels).expanduser().resolve()
    if not LABELS_PATH.exists():
        sys.exit(f"ERROR: labels file not found: {LABELS_PATH}")
    if charts_dir is not None:
        CHARTS = Path(charts_dir).expanduser().resolve()
        if not CHARTS.exists():
            sys.exit(f"ERROR: charts-dir not found: {CHARTS}")
    if assets_dir is not None:
        ASSETS = Path(assets_dir).expanduser().resolve()
        if not ASSETS.exists():
            sys.exit(f"ERROR: assets-dir not found: {ASSETS}")
    if outline_md is not None:
        V8 = Path(outline_md).expanduser().resolve()
        if not V8.exists():
            sys.exit(f"ERROR: outline-md not found: {V8}")
...
def parse_labels():
    """Load branding labels from LABELS_PATH (YAML)."""
    import yaml as _yaml
    data = _yaml.safe_load(LABELS_PATH.read_text(encoding="utf-8")) or {}
    if "labels" not in data or not isinstance(data["labels"], dict):
        sys.exit(f"ERROR: {LABELS_PATH} must have a top-level `labels:` dict")
    return {str(k): str(v) for k, v in data["labels"].items()}


LABELS = None  # populated by configure(); raises if accessed before configure()
...
def main():
    parser = argparse.ArgumentParser(description="Build V8 deck pptx from canonical .md outline.")
    ...
    parser.add_argument("--outline", required=True, help="Path to deck-outline-v8.md")
    ...
    configure(template=args.template, labels=args.labels, outline_md=args.outline, output=args.output)

    rows = parse_v8()
    print(f"Parsed {len(rows)} V8 rows\n")

    if args.part != "all":
        pattern, _ = PART_FILTERS[args.part]
        before = len(rows)
