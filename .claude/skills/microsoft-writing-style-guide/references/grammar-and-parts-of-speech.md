...
**Examples** The Windows Start screen is uniquely yours, personalized with your favorite apps, people, photos, and colors. Windows Update installs important updates automatically.
...
**Examples** Check if you have local admin rights.
...
**Examples** Notify me when a new Bluetooth device tries to connect to my computer
...
**Examples** Notify scheduled default setting is the easiest way to keep your computer up to date. (Instead of *We recommend that you use the scheduled default setting ....*)
...
Capitalize technology terms as proper nouns only when:
  ...
  - You
need to distinguish a component or product, such as SQL Server, from a general technology with a similar name, such as an SQL database server.
...
If you're not sure whether a term is a proper noun (and thus capitalized) or a common noun (lowercase), check *Merriam-Webster Dictionary,* and the A–Z word list. Default to lowercase unless there's a compelling reason to capitalize the term.
...
Collective nouns like *company* take a singular pronoun. Don't use a plural pronoun (like *they*) for a collective noun.<br /> **Examples**<br />Meet with up to 250 people. All they need is a phone or internet connection.<br />The company upgraded its cloud storage solution to Microsoft Azure.
...
The prepositional phrase, *of the selected message,* describes the noun, *content.*
...
**Examples** Use a different instrumentation key for each environment that your application runs in. Specify which event hub you want to send the data to.

## Prepositions with UI elements

Whether you use *in* or *on* as the preposition for a UI element depends on what the element is.
...
**Examples** In the **Server name** box, enter the name that you copied earlier. In the **Create a deployment** pane, enter resource details. On the ribbon, select **Options**. On the toolbar, select **File** > **Save**.
