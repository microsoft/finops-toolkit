...
Alphabetical Microsoft style guidance for individual words and terms. Each entry gives preferred usage, prohibited usage, and notes.
...
In most situations, avoid using *navigate* (verb) and *navigation* (noun or adjective) because they're formal words that might not localize well. Use simpler words instead.

**See also** browse Describing interactions with the UI

### Alternatives to *navigate*

| To describe | Use | Example |
| ----------- | --- | ------- |
| Going from place to place on the internet or an intranet | *browse* | To browse the web, select any link on your home page. |
| Going directly to a webpage or website, whether by entering a URL or by selecting a link | *go to* | To go to a webpage, enter the address of the page on the address bar, and then select **Go**. |
| Going to a folder or file | *browse* if the user has to find it<br><br>*go to* if the action is direct | Go to your **Downloads** folder and open the file. |
| Moving through a page, file, or app | *move through* | Use the **Next** buttons to move through the pages. |

### Alternatives to *navigation*

Avoid referring to UI controls and buttons as *navigation buttons* and to webpages that orient the customer as *navigation topics* or *navigation pages*. Most of the time, the *navigation* part is unnecessary and you can simply use *buttons*, *topics,* and *pages*.

Don't use *navigation pane* or *navigation menu* unless the term appears in the UI. Use *left pane* or *left menu* instead if the use of a direction is appropriate.

If you need to use *navigation*, never abbreviate it to *nav* (as in *nav pane* or *left nav*) because it's slang.

**Examples** Use the buttons to make your selection. On the service menu, select your service. Then, on the left pane, select your resource.

You don't need to find an alternative to *navigation* when it's the most precise word in the context.

**Example** This change improves code organization and navigation.
...
**Example** If you want to use a laser printer, you need a laser printer driver.
...
Don't hyphenate words beginning with *non-,* such as *nonnumeric* and *nonzero,* unless a hyphen is necessary to avoid confusion, as in *non-native,* or *non-* is followed by a proper noun, as in *non-English.* When in doubt, check *Merriam-Webster Dictionary.* Don't use *non-* to negate an entire phrase.

**Example** unrelated to security (instead of *non-security related*)
...
**Examples** Some text extends into the nonprintable area of the page. Select **Show/Hide** to display  all nonprinting characters, including paragraph marks and space marks.
...
**Examples** When you receive a message, a notification briefly appears at the top of Start ... If you get a notification, tap it before you unlock the phone and you'll be automatically taken to the notification.
...
**Example** (on X, formerly known as Twitter) A quick look at top conversations and \#Office365 product announcements of the week.
...
**See also** key result (KR).md), key performance indicator (KPI).md)
...
The term *on demand* describes something that's available or provided whenever it's requested or needed.

Don't hyphenate *on demand* when you're using it as an adverbial phrase after a verb.

**Examples** Watch on demand. Provision your resources on demand. You can access reports on demand from the dashboard.

Hyphenate *on-demand* when you're using it as a adjective that comes before a noun.

**Examples** Watch the on-demand webinar. Use on-demand compute resources to scale quickly. The service provides on-demand reporting.
...
**Example** After you save the document, you can close the app.
...
**Examples** Search Help for more information. Check online for more up-to-date information.
...
*Opt in* is a general term for an action that a user takes to subscribe to, sign up for, or start to receive information about a service, app, or site, or to give consent for data collection, processing, and storage.

*Opt out* is a general term for an action that a user takes to avoid subscribing, signing up, or giving consent, or to unsubscribe, cancel a sign-up, or withdraw consent.

Unless space is limited or you're referring to the opt-in or opt-out process in a general way, use a more precise term that gives a clearer idea of what users must do.

**Examples** Sign up to receive our newsletter. Select the checkbox to agree to legal terms and conditions. To give consent for data collection or use, turn on the toggle. To unsubscribe, select the link.

If you must use the terms:

- Use *opt in* and *opt out* as verb phrases. Write *opt in to*, not *opt into*.
- Use the hyphenated forms *opt-in* and *opt-out* as adjectives. Don't use them as nouns.

**Examples** If you want to opt in to our offer program, follow the link. Complete the opt-out form. Opting out isn't an option.
...
**Examples** Select **No header row.** When you select the **Keep source formatting** option, .... For **Paste options**, select **Keep text only**.
...
**Examples** Get an online org chart application that works with Azure Active Directory and Office 365. How to prepare for your cross-org Exchange migration
...
**Examples** Print a document to a file or to a specific printer. The **HttpResponse.Output** property delivers text output to the outgoing HTTP response stream.
...
**Examples** A transparent viewing layer with a red border appears over the diagram. The installed base is more than 2 million.
...
**Examples** In **Colors**, let Windows pull an accent color from your background, or choose your own color. In the **Color** palette, choose a color for the object outline.
...
Use *pane* to refer to the separate areas of a split window. For example, a list of resources might appear on one pane and the contents of a selected resource on another pane.

If a pane isn't labeled in the UI, use lowercase for the name that describes it. If a pane is labeled in the UI, use bold on the name.

Use the preposition *in* when you refer to a pane.

Use *open* and *close* as verbs with panes.

**Examples** Open the annotation pane. In the **Score** pane, select an asset to view details.

If you're creating UI text, use sentence-style capitalization for pane names.
...
**Examples** Learn why getting rid of passwords is important. Before you can do away with passwords, you need something to replace them. See how a password-free environment works. Sign in without a password. An authentication solution that doesn't require passwords
...
**Example** The full path of my current tax form is: C:\\Documents and Settings\\user1\\My Documents\\Taxes\\This year's taxes
...
**Example** (Mac) Macintosh HD:My Documentation:Sales CORP-16:TOMCAT:EX130D Mac Temp:Workbook1
...
**Examples** Customers who sign in only once a day are rare. You can have only one drive letter per network resource.

Don't use *per* to mean *by* or *in accordance with*.

**Examples** Find all the articles that contain a specific word by following the instructions on your screen. Identify your computer by using the procedure in the next section.
...
**Examples** Setting the Traverse Folder permission on a folder doesn't automatically set the Execute File permission on all files within that folder. Grant Read, Read and Execute, and List Folder Content permissions to the Users group. Whenever possible, assign permissions to groups instead of users.
...
**Examples** When you pin an app, it's added to the Start menu as a new tile. Pin important messages to the top of the message list, and unpin them when you're done with them.
...
It's OK to use *p*<em>ing</em> to refer to using the PING protocol, which is used to determine the presence of a host on the internet. Don't spell out PING. If necessary, describe it as Packet Internet Groper.
...
**Examples** Plug and Play functionality Plug and Play printer Plug and Play records the information in an event log.

**See also** UPnP

### plugin

A plugin is a component that adds specific abilities to an application. In generative AI experiences, a plugin is a component that extends what a language model or an agent can do by giving it access to capabilities in Microsoft and non-Microsoft services and sites through APIs.

In either case, use one word (no hyphen).

Don't use as a synonym for *add-in* or *add-on*.
...
**Examples** Microsoft Dynamics 365 and Office 365 are better together. Choose from more than 750,000 apps in Microsoft Store. 999+ connections (in the UI)
...
**See also** start, restart shut down, shutdown turn on, turn off
...
In general, don’t hyphenate words beginning with *pre-,* such as *preallocate* and *preempt,* unless it’s necessary to avoid confusion, as in *pre-engineered,* or if *pre-* is followed by a proper noun, as in *pre-C++.* When in doubt, refer to *Merriam-Webster Dictionary.*
...
**See** Describing interactions with UI Touch and pen interaction term collection Keys and keyboard shortcuts term collection
...
**Examples** Office 2016 Preview Windows 10 Insider Preview Microsoft Power BI Preview A preview of Azure Container Service is available.
...
**Examples** Your document will be sent to the printer. Your file will be added to the list of documents waiting to be printed.
...
**Learn prompting** If you want quality outputs, you need fine-tuned prompts. To start becoming a prompting pro, check out this prompt crafting guide (for marketing content) from Writer.com. Additional guidance we like:
...
**Examples** In the **Background** list, select **Solid color**. Go to **Tools**, and select **Change language**.

**See also** dropdown Describing interactions with UI Formatting text in instructions
...
**See also** option, option button Describing interactions with UI Formatting text in instructions
...
In general, don’t hyphenate words beginning with *re-* unless it’s necessary to avoid confusion or *re-* is followed by a proper noun. When in doubt, refer to *Merriam-Webster Dictionary.*

**Examples** reenter recover (to get back or regain) re-cover (to cover again) recreate (to take part in a recreational activity) re-create (to create anew)
...
**Examples** read-only memory This file is read-only.
...
The term *real time* describes something that's diplayed or reported as an event is happening.

The noun form is two words and typically follows the preposition *in*. Hyphenate as an adjective that precedes a noun.

**Examples**  
Respond quickly to security events by monitoring them in real time.  
Respond quickly to security events by using real-time monitoring.

**See also**  
[near real time, near-real-time](~/a-z-word-list-term-collections/n/near-real-time.md)
### reboot
...
**Examples** Windows 10 requires 16 GB of hard drive space for 32-bit OS and 20 GB for 64-bit OS.<br /> Small storage devices, like devices with 32-GB hard drives or older devices with full hard drives, might require additional storage to complete the upgrade. If you have limited hard drive space available, we recommend that you have a USB flash drive handy when you install Windows 10.
...
**Examples** **HKEY\_CLASSES\_ROOT** **HKEY\_LOCAL\_MACHINE**
...
**Examples** **SOFTWARE** **ApplicationIdentifier** **CurrentVersion**

Refer to an entire subkey path as a *subkey,* not a *path*.

**Example** (typical subkey ) **\\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Policies\\Microsoft\\SystemCertificates\\AuthRoot**
...
Use *remove:*
  ...
  - To
describe taking a toolbar button off a toolbar or hiding displayed data, such as columns in a spreadsheet, without deleting the data.
...
**Examples** To replace all instances of a word or phrase at one time .... The **Replace** method replaces text found in a regular expression search.

Use *replace* instead of *overwrite*.

**Examples** Replace the selected text with the new text. Replace the file with the changed file.
...
**Examples** Restore the file. Restore the window to its minimized state. For more information about System Restore ....
...
**Examples** Under the full recovery model, first restore one or more data backups, and then restore the subsequent log backups to roll the database forward in time. Restore the database. Use the RESTORE statement to specify the restore operation.
...
**Examples** Right-click to select the file. Using the right mouse button (right-click) …. Select and hold (or right-click) the Windows taskbar, and then select **Cascade windows**.
...
**Examples** To run the program, select **Execute**. Commands are run in the order in which they're listed in the file.
...
**Examples** To install Exchange 2016, your computer must be running the full installation of Windows Server. Office 2016 runs on the following versions of Windows: Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2, Windows Server 10, Windows 7 Service Pack 1, Windows 8, Windows 8.1, Windows 10.
