...
**Examples** To add rows or columns to a table, .... To add one or more rows or columns to a table, ....
...
**Examples** This displays a sample of what the control will look like with the scheme applied. To preview what the control will look like with the scheme applied, select **Preview**.
...
A screen reader is assistive software that renders text and image content as speech or braille output.

When the term is a noun, it's two words. When you're using it as an adjective before a noun, hyphenate it for clarity. Never use a one-word form.

**Examples** The screen reader reads the contents of the screen aloud for users who are blind or have low vision. The app is designed for screen-reader compatibility.

**See also** Writing for all abilities Colors and patterns in text, graphics, and design Alternative text (alt text)
...
**Examples** Move through the folder until you see the file you want. Move the wheel button on the Microsoft Arc Touch Mouse to scroll up and down in a document.
...
See Accessibility guidelines and requirements for more information about creating products and content that are accessible to people of all abilities.
...
**Examples** A service-level agreement (SLA) is an agreement between a service company and a service customer. An SLA defines for both the company and the customer when a service job should be completed.
...
**Examples** Select a color for an appointment or a meeting. Enter a location for **Tab stop position**.
...
**Examples** Unpack everything before you set up your computer. The setup time is about 15 minutes. Your office setup should be ergonomically designed. Run Setup before you open other apps.
...
**Examples** The command isn’t available until you select text. If the option appears dimmed, it's not available. Selected effects appear shaded.
...
**Examples** Use **Alt+Spacebar** to display the shortcut menu for the active window. Right-click the selected text, and then select **Copy**. Select the text, open the shortcut menu, and then select **Copy**.

**See also** Describing interactions with UI Keys and keyboard shortcuts term collection pop-up
...
**Examples** You'll need an internet connection to install the upgrade. We recommend that you wait for us to notify you when we've confirmed that your PC is ready. Free technical support is available when you register with Microsoft.
...
**Examples** You should back up your data periodically. If you don't use automatic updating, you should check for updates yourself at least once every week. If the anti-malware app is compatible and current, it will be preserved during the upgrade.

Use *must* only to describe a required action.

**Example** To save copies of a document in the same location, you must save each copy with a different file name.
...
**Examples** To turn off your computer, select **Settings**, and then select **Shut down**. Use the **Shut down** command to safely close open apps, shut down Windows, and turn off the device. Many computers turn the power off automatically. The accidental shutdown might have damaged some files.

**See also** Describing interactions with UI turn on, turn off sleep
...
**Examples** Enter your password to sign in. Learn how to sign in to Power BI service on the web and how to sign out of it. After an extended period without activity, the network will sign you out automatically.
...
**Examples** To publish your files to the web, select **Publish to the web**. When you see a clip that you like, drag it into your document.
...
Avoid using the abbreviation *SKU* (pronounced "skew") to describe an offering type for a product, because it's jargon. Instead, use a term that's precise and well understood, like *subscription*, *edition*, *version*, or *tier*.

If you need to use *SKU* because customers expect it, you don't need to spell it out. If you choose to spell it out, use *stock-keeping unit (SKU)* at first mention.

**Examples** Pricing depends on the subscription level. Choose a storage tier. For **SKU**, select an option. Remember that **Standard** has more features than **Basic**.
...
**Example** Move the slider downward to decrease the volume.
...
Error messages should apologize only for things that went wrong with the Microsoft product. Don't apologize for problems that are outside the product, such as a broken link or waiting for a network connection to be found. In those cases, be direct about what went wrong.

**Examples** We're sorry, but we detected a problem, and your PC was shut down to protect your files and other data. Sorry, the service isn't available right now. This task couldn't be completed. Please try again. You're not connected. Let's get you back online. The page can't be reached. This page didn't load properly.
...
**Examples** Use the Spelling and Grammar tool to check spelling in the document. Check the spelling in the document. Use a spell checker to find spelling errors in your document.
...
**Examples** To open the app, select the icon. When the app opens, a startup screen appears. Open OneNote and start taking notes.
...
In general, don’t hyphenate words beginning with *sub-,* such as *subheading* and *subsection,* unless it’s necessary to avoid confusion or if *sub-* is followed by a proper noun, as in *sub-Saharan*. When in doubt, check *Merriam-Webster Dictionary.*
...
In general terms, a subnet (short for *subnetwork*) is a segment of a network. For technical audiences, you can use *subnet* without explaining that it's an abbreviation.
...
You can abbreviate *synchronize* or *synchronization* to *sync* for a friendlier tone, if it works well in the context. For example, a user can sync a file, folder, or library with a local drive, a SharePoint drive, a server, or the cloud. A user can also sync settings, messages, and other data across devices, apps, and accounts.

Abbreviations of *synchronize* and *synchronization* don't save space when they're localized. In languages other than English, localization uses the unabbreviated forms.

**Guidelines**

- Don't include *h* in the abbreviation. Use *sync*, *syncs*, *synced*, and *syncing*, not *synch*, *synchs*, *synched*, or *synching*.

- In general, use the phrase *sync with* instead of *sync from* or *sync to*. If information is being synced to and from two sources, describe it as syncing between or syncing across.

- Don't use *sync* to abbreviate *synchronous*.
...
**Examples** Use the **Tab** key to move through a dialog box. Set a tab stop on the ruler. On the **View** tab ....
...
**Examples** Set your phone to automatically save photos and videos to Microsoft OneDrive. By default, the file is saved to the Scanned documents folder. Copy the file to the USB drive.
...
**Examples** **File name** box Enter or update an email address for **Work email**.
...
**Examples** Thanks for your feedback. Microsoft offers a 10 percent military discount as a way to say thank you for your service.
...
**Examples** To learn more about features that aren’t supported in the trial version, see the feature comparison chart. Catch the latest episode of “This Week on Windows,” which looks at the new Surface Book and more. The comparison chart shows which features aren't supported in the trial version.
...
**Examples** items on your to-do list Here's what you need to do.
...
**See also** add-in, add-on app, application applet Control Panel
...
A tooltip is a small pop-up note that provides information about a UI element (such as a control, icon, or link) when a user hovers over it or selects it.

The term *tooltip* is one word and lowercase. Don't spell it as *ToolTip*.

The guidelines for creating tooltips vary by product. Here are general principles:

- Use them sparingly because they can be distracting.
- Be brief.
- Make sure that the information is helpful as a tip and doesn't just repeat what a label shows.
- Use sentence-style capitalization. (Capitalize only the first word and proper nouns.)
- If the tooltip is a fragment, don't end it with a period. If it needs to read like a full sentence, end it with a period.
...
**Examples** To turn on pop-up blocking …. To keep all applied filters, turn on the **pass all filters** toggle. If the command prompt isn’t available, your network administrator might have turned off this feature.

**See also** Describing interactions with UI disable, disables (functionality) enable, enables shut down, shutdown unavailable
...
In general, don’t hyphenate words beginning with *un-,* such as *undo* and *unread,* unless it’s necessary to avoid confusion, as in *un-ionized,* or unless *un-* is followed by a proper noun, as in *un-American.* When in doubt, check *Merriam-Webster Dictionary.*
...
**Examples** The command isn’t available until you select text. If the option appears dimmed, it's not available. If the command prompt isn’t available, your network administrator might have turned off this feature. Some My Site features are temporarily unavailable. Please try again later.
...
**Examples** To undo multiple actions .... After undoing an action, ....
...
**Examples** Upgrade qualified Windows 7 or Windows 8.1 devices for free. The upgrade was successful.
...
**Examples** Change all the lowercase letters to uppercase. Change the case of both lowercase and uppercase letters.
...
A uniform resource identifier (URI) is a series of characters that identify a resource. A URL is a type of URI.

**Guidelines**

- In content for a general audience, use *resource ID* rather than *URI* unless you need to match the interface.

- In content for a technical audience, you don't need to spell out *URI* on the first mention.

- If you need to spell out the term, use lowercase.

- Use *a,* not *an,* as an article before *URI*.

- Use *of* (not *for*) to describe the relationship of the word *URI* to a resource. 

**Example** Copy the URI of the resource that you created.
...
**Examples** Refine your pictures by using the photo editor. Azure DNS is a hosting service that uses Azure infrastructure to provide DNS name resolution.
...
**See** add-in, add-on app, application applet tool Control Panel
