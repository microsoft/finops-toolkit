...
>   - **msexports_Transform** – Converts the CSV export to parquet and moves it to the **ingestion** container (same path), overwriting the existing contents for that month. [Learn more](../../src/modules/pipelines/README.md#msexports_transform).
>
...
>
> - **msexports_Setup** – Monitors the **config/settings.json** file for changes and creates, updates, or deletes exports based on changes. Also handles data backfill when needed for new scopes based on the data retention setting. Exports are created with a `FinOpsHubs_` prefix to be easily identifiable. [Learn more](../../src/modules/pipelines/README.md#msexports_setup).
>
