...
Data is at the heart of every FinOps practice. Whether you're a hobbyist or part of a large team, you can't "FinOps" without data. And when you start to "FinOps" at scale, you realize you need a lot of different data from a lot of different data sources. And these data sources often have different characteristics and change over time. Luckily, FinOps hubs were designed to handle ever-evolving datasets through its "schema version" concept.

FinOps hubs schema versions are contracts you can rely on to always present a consistent version of the data. Today, FinOps hubs use a "v1_0" schema version that aligns with FOCUS 1.0 (hence the name). As of FinOps hubs 12, you will see a new "v1_2" schema version, which aligns with FOCUS 1.2. And you'll notice that your data is populated in the FOCUS 1.2 format regardless of what dataset version you exported from Cost Management. And the best part is, this optimization won't impact you at all as long as your reports and integrations are set up to use the versioned functions (e.g., Costs_v1_0). If you find you have code using non-versioned functions (e.g., Costs), simply switch to the versioned function to get all data – old and new – using the previous v1_0 schema version.
...
> This month, you'll find support for FOCUS 1.2, autostart in FinOps hubs which can reduce your hub costs, a new page in the Cost summary Power BI report, and various small fixes, improvements, and documentation updates across the board.

## Social media

**LinkedIn:**

> FinOps toolkit 12 is here with FOCUS 1.2 support, Data Explorer auto-start to cut hub costs, a new executive summary page in Cost summary, and Parquet export support in PowerShell. {{POST_URL}} #FinOps #Azure #FOCUS

**Twitter/X:**

> FinOps toolkit 12: FOCUS 1.2, Data Explorer auto-start, new Cost summary executive page, Parquet exports in PowerShell. {{POST_URL}} #FinOps #Azure

**Internal:**

> FinOps toolkit 12 is live – FOCUS 1.2, hub auto-start, executive cost summary, and Parquet export support. {{POST_URL}}
