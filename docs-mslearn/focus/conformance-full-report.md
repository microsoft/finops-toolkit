...
- **MAY** requirements are optional and generally used to prepare FinOps practitioners for edge cases.

While there is no official measurement for FOCUS conformance, we have evaluated a conformance score of **95%**, which accounts for all fully and partially supported requirements. The following table summarizes requirements by level of support.
...
| **RECOMMENDED** |     0     |        1        |       1       |       0        |
| **MAY**         |    22     |        0        |       0       |       1        |
| Summary         |   91.7%   |      6.6%       |     1.6%      |                |

<br>

## How this document is organized

The sections below include a detailed list of requirements, their level of support in the Microsoft Cost Management FOCUS 1.0 dataset, and applicable notes for each. For a high-level summary of the gaps, refer to the [FOCUS conformance summary](./conformance-summary.md). IDs provided in the sections below are for reference purposes only. IDs are not defined as part of FOCUS.

The rest of this document lists the FOCUS requirements grouped by attribute and column. [Columns](#columns) define the specific data elements in the dataset and [attributes](#attributes) define how columns and rows should behave. High-level descriptions and a link to the original requirements are included in the sections below.
...
| CNO2   | MUST   | All custom columns MUST be prefixed with a consistent `x_` prefix to identify them as external, custom columns and distinguish them from FOCUS columns to avoid conflicts in future releases. | Partially Supports | `BillingAccountType` and `SubAccountType` were unintentionally introduced in Microsoft's FOCUS 1.0 preview dataset version without the `x_` prefix. Both columns are documented in a pending FOCUS pull request. We are maintaining these column names for backwards compatibility until an official determination is made about them. |
...
| BPE2.2 | MUST        | This column... MUST be an exclusive value...                                                                                                                   | Does Not Support | `BillingPeriodEnd` has a known bug where the time is "23:59:59.999" when it should be "00:00:00". |
...
| CnC5   | MUST        | The ContractedCost of a charge unrelated to other charges (e.g., when the ChargeCategory is "Credit") MUST match the BilledCost.                                                                                                                                  | Partially Supports | As of October 2024, `ContractedCost` uses the wrong scale for EA accounts. Multiply by `x_PricingBlockSize` to resolve. `ContractedCost` may be off by less than 0.00001 due to rounding errors. |
...
| EC2.1 | MUST        | This column MUST be of type Decimal...                                                                                                                                                    | Supports |       |
...
| LC3   | MUST        | When ListUnitPrice is present and not null, multiplying the ListUnitPrice by PricingQuantity MUST produce the ListCost, except in cases of ChargeClass "Correction", which may address PricingQuantity or any cost discrepancies independently. | Partially Supports | As of October 2024, `PricingQuantity` uses the wrong scale for EA accounts. Multiply by `x_PricingBlockSize` to resolve. After adjusting for this, `ListCost` may be off by less than 0.0000000001 due to rounding errors. |
...
| LUP2.3 | MUST        | This column... \[MUST\] be denominated in the BillingCurrency.                                                                                                                                                                           | Supports           |                                                                                                              |
...
| PQ5   | MUST        | When unit prices are not null, multiplying PricingQuantity by a unit price MUST produce a result equal to the corresponding cost metric, except in cases of ChargeClass "Correction", which may address PricingQuantity or any cost discrepancies independently. | Partially Supports | See LC3 and CnC3.                                                     |
...
| SkPI5.1 | MUST        | This column MUST NOT be null when ChargeClass is not "Correction" and ChargeCategory is "Usage" or "Purchase"...                | Supports           |                                                                                                                                                                                                                        |
| SkPI5.2 | MUST        | This column... MUST be null when ChargeCategory is "Tax"....                                                                    | Not Applicable     | Taxes are not included in any Cost Management cost and usage dataset.                                                                                                                                                  |
| SkPI5.3 | MAY         | This column... MAY be null for all other combinations of ChargeClass and ChargeCategory.                                        | Supports           |                                                                                                                                                                                                                        |
