...
| Column         | ID                                                 | Gap              | Description                                                                                                                                                                                    |
| -------------- | -------------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BillingPeriodEnd    | [BPE2.2](#billing-period-end) | Incorrect data  | BillingPeriodEnd` has a known bug where the time is "23:59:59.999" when it should be "00:00:00". |
| ContractedCost      | [CnC5](#contracted-cost) | Incorrect data | `ContractedCost` uses the wrong scale and is incorrect for Enterprise Agreement accounts.<br><br>Workaround: Multiply the `ContractedCost` by `x_PricingBlockSize`. |
| PricingQuantity     | [PQ5](#pricing-quantity), [CnC3](#contracted-cost), [LC3](#list-cost) | Incorrect data | `PricingQuantity` uses the wrong scale and is incorrect for Enterprise Agreement accounts.<br><br>Workaround: Multiply the `PricingQuantity` by `x_PricingBlockSize`. |
...
| CommitmentDiscountId | [DH3.12](#discount-handling) | Incorrect casing | `CommitmentDiscountId` does not match `ResourceId` exactly because `ResourceId` is lowercased.<br><br>Workaround: Convert `CommitmentDiscountId` to lowercase if comparing to `ResourceId`. |
