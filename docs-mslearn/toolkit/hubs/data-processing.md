...
The following steps happen when a new, managed scope is added to a hub instance. Unmanaged scopes (where Cost Management exports are manually configured) do not require any setup in hubs.
<!--
```mermaid
sequenceDiagram
    config->>config: ① config_SettingsUpdated
...
1. Power BI or other tools read data from the **ingestion** container.
   > [!NOTE]
   > Power BI support for Azure Data Explorer is coming in a future update.
