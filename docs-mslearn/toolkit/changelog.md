...
- **Fixed**
  - Fixed hub redeploys silently resetting ingestion and Data Explorer retention to the 13-month default when a custom value wasn't explicitly re-specified, which caused the purge pipeline to age out historical data on the next run ([#2206](https://github.com/microsoft/finops-toolkit/issues/2206)).
