...
- **Fixed**
  ...
  - Fixed the Azure Resource Graph query pipeline only ingesting the first 1,000 rows of any query (for example, `Recommendations-Microsoft-AdvisorCost`), since Azure Resource Graph caps a single response at 1,000 rows and returns a `$skipToken` for the rest. The pipeline now loops until Azure Resource Graph stops returning a token, writing each page to its own Parquet file ([#2320](https://github.com/microsoft/finops-toolkit/issues/2320)).
