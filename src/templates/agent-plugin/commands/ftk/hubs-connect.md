...
If a FinOps hub instance was identified in a previous step, run the following query with the `azure-mcp-server` command `kusto_query` to validate the FinOps hub instance. Pass the selected `clusterUri` as `cluster-uri`, use `Hub` as the database, and use the tenant and subscription from the selected environment:
