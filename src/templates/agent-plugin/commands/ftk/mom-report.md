...
## 2 - Plan Phase
1. Plan ahead in `ftk/planning/plan-[environment-name]-report-[date].md`
2. Track progress in `ftk/planning/progress-[environment-name]-report-[date].md`
3. Save/update the report in `ftk/results/[environment-name]-report-[date].md`.
4. Do not save query results anywhere except in `ftk/results/[environment-name]-report-[date].md`.

**Checkpoint:** Present the plan and confirm it covers the right scope before executing.

## 3 - Execute Phase
1. You may encounter errors along the way which you will need to troubleshoot — check your `ftk/notes/` to avoid troubleshooting the same issue unnecessarily.
2. Check casting, syntax, and query structure. Make sure to use the correct data types and parameters for functions and tools.
3. Document issues and solutions in `ftk/notes/topic-name.md`.
4. Add new working queries you create to `skills/finops-toolkit/references/queries/catalog/query-name.kql` and update `skills/finops-toolkit/references/queries/INDEX.md` for re-use.
5. Use autonomous batch processing to handle large datasets efficiently.
6. Save your work as you go to `ftk/results/[environment-name]-report-[date].md` to avoid lost work.
7. Investigate suspicious workload patterns using `skills/finops-toolkit/references/finops-hubs.md` for anomaly, budget, and optimization context.
8. Explore material patterns beyond the obvious cost drivers, then summarize findings and stop when the report is complete or blocked.

**Checkpoint:** Update the report and summarize key findings so far before moving to reflection.

## 4 - Reflect Phase
1. Use the bundled reference guidance in `skills/finops-toolkit/references/` to interpret `ftk/results/[environment-name]-report-[date].md` and validate whether the month-over-month story is evidence-backed.
2. Make the report professional, scannable and colorful. Use charts, graphs and emojis.
3. Check your work as you go for errors and omissions. Make sure the report is complete and renders correctly.
