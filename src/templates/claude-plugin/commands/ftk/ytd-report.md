...
2. Read and review the knowledge base to build comprehensive context:
   ...
   - Use `ftk/knowledge/core/finops-framework-complete-index.md` for foundational FinOps concepts
   - Use `ftk/knowledge/practices/index.md` for implementation guidance
   - Use `ftk/knowledge/analysis/finops-hub-overview.md` for data analysis insights
...
## 2 - Plan Phase
3. Plan ahead in `ftk/planning/[environment-name]-[date]-plan.md`
4. Track progress in `ftk/planning/[environment-name]-[date]-progress.md`
5. Save/update the report in `ftk/results/[environment-name]-[date]-report.md`.
6. Do not save query results anywhere except in `ftk/results/[environment-name]-[date]-report.md`.
...
9. Reference `ftk/knowledge/queries/tool-azmcp-kusto.md` for proper Azure Data Explorer tool usage and parameter requirements.
...
13. Save your work opportunistically to `ftk/results/[environment-name]-[date]-report.md` to avoid lost work.
14. Investigate suspicious workload patterns using guidance from `ftk/knowledge/analysis/` for anomaly and fraud detection.
15. Leave no stone unturned. Explore the data. Look for more than just the usual suspects.

## 4 - Reflect Phase
16. Use comprehensive knowledge from the knowledge base to interpret results and validate findings against `ftk/results/[environment-name]-[date]-report.md`
