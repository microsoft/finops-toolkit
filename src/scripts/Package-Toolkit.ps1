...
function Open-PowerBIProjects()
{
    # Recreate temp pbix folder
    Remove-Item -Path "$relDir/pbix" -Recurse -Force
    & "$PSScriptRoot/New-Directory" "$relDir/pbix"

    $pbi = Get-ChildItem -Path "$PSScriptRoot/../power-bi" -Include "*.pbip" -Recurse
    if ($OpenPBI)
    {
...
{
    # Clean PBIX files
    Get-ChildItem -Path "$relDir/pbix/*.pbix" `
    | ForEach-Object {
        # Expand PBIX files for cleanup
        $pbixDir = $_.FullName.Replace('.pbix', '')
        Expand-Archive -Path $_ -DestinationPath $pbixDir
