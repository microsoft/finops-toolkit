...
The [InstanceSizeFlexibility.csv](./InstanceSizeFlexibility.csv) file maps each ARM SKU to its instance size flexibility (ISF) group and ratio. ISF lets a reservation apply across multiple SKUs in the same flexibility group; the ratio is the relative weight of each SKU within its group. It covers Virtual Machines, Redis Cache, and Dedicated Host, unioned across every physical region the ARM locations API reports (ISF groups are region-stable, but SKU availability is not, and new GPU and specialty SKUs often launch in only a handful of regions). The Microsoft ratios are kept as-is (they don't always start at `1` for the smallest SKU); pass `-Normalize` to rescale each group so its smallest SKU is `1`. This data is updated weekly by the [Update Instance Size Flexibility](../../.github/workflows/opendata-instance-size-flexibility.yml) workflow, which queries the [Azure Reservations Catalogs API](https://learn.microsoft.com/azure/cost-management-billing/reservations/instance-size-flexibility#extract-instance-size-flexibility-ratios-using-azure-catalogs-api).

The dataset is additive. The Catalogs API only returns SKUs that are still purchasable, so a destructive write drops retired series (Av2, D, DS, Dv2, F, G, H, NC, NV, and others) that customers still hold reservations against ([#2300](https://github.com/microsoft/finops-toolkit/issues/2300)). Each run merges the API result over the previously published file, keyed on `ArmSkuName`, so the live API wins wherever it has an opinion and everything else carries forward. The published file is therefore the floor for those groups, and the run fails rather than publishing a dataset smaller than the last one.

Ratios are only comparable within a flexibility group, and the ratios the Catalogs API returns are used verbatim.

The retired files were on a different scale: they normalized each group so its smallest SKU was `1`, while the API leaves them unnormalized -- only 52 of its 211 groups start at `1`, against 432 of 433 in the retired files. Both encode the same proportions, differing by a per-group constant (usually 2×, e.g. `Standard_DS11_v2` is `1` in the retired file and `2` in the API). Because Azure retires individual sizes rather than whole families, the API often covers only part of a group — 44 of the 204 groups both sources knew — so a naive merge would leave one group holding both units. The rows backfilled from the retired files were therefore converted onto the API scale once, using the factor derived from the SKUs each group shared with the API, and the generator fails the run if a carried-forward group is ever found on the wrong scale again.

The fully retired groups (roughly a third of the dataset — `Av2 Series`, `D Series`, `F Series`, and the like) have no API counterpart to convert against, so they keep the normalized scale. That is harmless as long as ratios are compared only within their own group.
...
SKUs whose ratio is not a positive number are dropped. The API returns one such row today (`azure_redis_cache_isolated_i100` at `0`); a zero carries no flexibility information and consumers divide by `Ratio`, so publishing it would hand them a division by zero.
